	...
	<c-panel>
		...
		<div class="field-column">
			...
			<span class="label">
				...
				<p class="label" style="display:inline-block;">
					Phrases or Words
				</p>
		...
		methods:{
			...
			onValidate_name : function(){
				if( this.form.name_string.length === 0) {
					this.$refs.name.className = 'text-area';
					return false;
				}
			...
			onValidate_name_check : function(){
				if( this.form.name_string.length > this.min_word_length ){
					this.$refs.name.className = 'text-area pass';
					return true;
				} else {
					this.$refs.name.className = 'text-area fail';
					return false;
				}
			},
...
.text-area.pass {
	border-bottom: 4px solid var( --colour-text-positive );
}
.text-area.fail {
	border-bottom: 4px solid var( --colour-text-negative );
